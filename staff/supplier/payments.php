                    ...
                    <table id="example2" class="table table-responsive table-hover">
                      <thead>
                       ...
                       <?php
                        ...
                        $query=$mysqli->query("SELECT * from supplier where email='$loggin_session'")or die($mysqli->error);
                        $row=$query->fetch_assoc();
                        
                          $sid=$row['id'];

                  $result=$mysqli->query("SELECT payments.id as id,payments.amount as amount,payments.payment_date as payment_date,payments.supplier_id as supplier_id,payments.transaction_code as transaction_code,products.product_img_name as product_img_name,products.product_name as product_name,supplier.firstname as supplier_firstname,supplier.company as supplier_company,payments.payment_status as payment_status from payments join products on payments.product_id=products.id join supplier on supplier.id=payments.supplier_id where payments.supplier_id='$sid' order by payment_status")or die($mysqli->error);
